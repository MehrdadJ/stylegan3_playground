...
class CaptureWidget:
    ...
    def dump_npy(self, image):
        ...
        try:
            ...
            if channels == 1:
                ...
                pil_image = PIL.Image.fromarray(image, 'RGB')
            np.save(os.path.join(self.path, f'{file_id:05d}.npy'), image)
        except:
            viz.result.error = renderer.CapturedException()

    def auto_save(self):
        viz = self.viz
        # Automatically set dump_image to True for triggering save
        self.dump_image = 'image' in viz.result

        if self.dump_image:
            self.dump_png(viz.result.image)
            self.dump_npy(viz.result.image)
            self.dump_image = False
    ...
    def __call__(self, show=True):
        ...
        ### Mehrdad
        # Auto save the visualization result image as a png and a numpy array every 500 milliseconds
        if self.disabled_time == 0:
            self.disabled_time = 0.5
            self.auto_save()
        ...
        if self.defer_frames > 0:
            ...
            if 'image' in viz.result:
                self.dump_png(viz.result.image)
                ### Mehrdad
                # Save the visualization result image as a numpy array
                self.dump_npy(viz.result.image)
            self.dump_image = False
        elif self.dump_gui:
